...
public class GeoShapesSearchAT extends BaseAT {
    ...
    public static void setUpSuite() {
        cassandraUtils = CassandraUtils.builder("search")
                                       .withPartitionKey("id").withClusteringKey()
    ...
    private void testCase(GeoShapeCondition geoShapeCondition, int search_case,String[] resultsForContains,String[] resultsForIntersects,String[] resultsForWithin) {
        cassandraUtils.filter(bool().must(geoShapeCondition.operation("contains"),match("search_case",search_case))).checkStringColumnWithoutOrder("identity",resultsForContains);
        cassandraUtils.filter(bool().must(geoShapeCondition.operation("intersects"),match("search_case",search_case))).checkStringColumnWithoutOrder("identity",resultsForIntersects);
        cassandraUtils.filter(bool().must(geoShapeCondition.operation("is_within"),match("search_case",search_case))).checkStringColumnWithoutOrder("identity",resultsForWithin);
    }
    @Test
