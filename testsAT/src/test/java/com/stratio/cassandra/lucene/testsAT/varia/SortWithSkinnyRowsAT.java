...
public class SortWithSkinnyRowsAT extends BaseAT {
    ...
    public void sortDesc() {
        utils.sort(field("rc").reverse(true)).limit(3).checkOrderedIntColumns("rc", 9, 8, 7);
    }

    @Test
    public void sortPartitionAsc() {
        utils.sort(field("rc").reverse(false)).andEq("pk", 1).limit(3).checkIntColumn("rc", 1);
    }

    @Test
    public void sortPartitionDesc() {
        utils.sort(field("rc").reverse(true)).andEq("pk", 1).limit(3).checkIntColumn("rc", 1);
    }
    ...
    public void sortTokenRangeAsc() {
        utils.sort(field("rc").reverse(false))
             .andGt("token(pk)", 0)
             .limit(3)
             .checkOrderedIntColumns("rc", 3, 6, 7);
    }
    ...
    public void sortTokenRangeDesc() {
        utils.sort(field("rc").reverse(true))
             .andGt("token(pk)", 0)
             .limit(3)
             .checkOrderedIntColumns("rc", 9, 7, 6);
    }
