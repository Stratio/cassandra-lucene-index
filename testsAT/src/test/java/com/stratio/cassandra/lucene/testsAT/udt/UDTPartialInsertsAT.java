...
public class UDTPartialInsertsAT extends BaseAT {
    ...
    public void testPartialInserts() {
        cassandraUtils
                ...
                .refresh()
                .filter(wildcard("address.street", "*"))
                .checkStringColumn("login", false, "USER2", "USER3")
                .filter(wildcard("address.city", "*"))
                .checkStringColumn("login", false, "USER2", "USER3")
                .filter(range("address.zip").lower(Integer.MIN_VALUE).upper(Integer.MAX_VALUE))
                .checkStringColumn("login", false, "USER2")
                ...
                .checkStringColumn("login", false, "USER2", "USER3", "USER4")
                .filter(range("address.zip").lower(Integer.MIN_VALUE).upper(Integer.MAX_VALUE))
                .checkStringColumn("login", false, "USER2", "USER4")
                .filter(match("address.zip", 94110))
                .checkStringColumn("login", false, "USER2")
    ...
    public void testPartialUpdates() {
        cassandraUtils
                ...
                .refresh()
                .filter(match("address.city", "London")).checkStringColumn("login", "USER5")
                .filter(match("address.zip", 12051)).checkStringColumn("login", "USER5")
                .filter(match("address.bool", true)).check(0)
                .filter(match("address.bool", false)).checkStringColumn("login", "USER5")
                .filter(range("address.point.latitude").lower(-1 * Float.MAX_VALUE).upper(Float.MAX_VALUE))
                .check(1)
                .filter(range("address.point.longitude").lower(-1 * Float.MAX_VALUE).upper(Float.MAX_VALUE))
                .check(1)
