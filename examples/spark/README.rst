...
The rest of worker machines need almost one Cassandra seeds IP address in order to form the ring so we proportionate the
CASSANDRA_SEEDS_IP with the worker1 IP address:
...
This example calculates the mean off all (1000 rows) temperature values.
