...
sed -i "s|^broadcast_rpc_address.*|broadcast_rpc_address: \$(hostname --ip-address)|" /etc/sds/cassandra/cassandra.yaml
sed -i "s|- seeds.*|- seeds: '\${SEEDS:=\$(hostname --ip-address)}'|" /etc/sds/cassandra/cassandra.yaml
sed -i "s|^listen_address.*|listen_address: \$(hostname --ip-address)|" /etc/sds/cassandra/cassandra.yaml
