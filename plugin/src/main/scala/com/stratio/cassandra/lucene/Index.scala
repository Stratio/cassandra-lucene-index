...
/** [[org.apache.cassandra.index.Index]] that uses Apache Lucene as backend. It allows, among others, multi-column
  ...
  */
class Index(table: ColumnFamilyStore, indexMetadata: IndexMetadata) extends org.apache.cassandra.index.Index {
  ...
  override def getMetadataReloadTask(indexMetadata: IndexMetadata): Callable[_] = () => {
    // TODO: Check return getBuildIndexTask if index metadata is different and this method is ever called
  ...
  override def dependsOn(column: ColumnDefinition): Boolean = {
    ...
    logger.trace(s"Asking if it depends on column $column")
  ...
  override def supportsExpression(column: ColumnDefinition, operator: Operator): Boolean = {
    logger.trace(s"Asking if it supports the expression $column $operator")
    service.supportsExpression(column, operator)
  }

  /** If the index supports custom search expressions using the {{{SELECT * FROM table WHERE expr(index_name,
    * expression)}}} syntax, this method should return the expected type of the expression argument. For example,
    * if the index supports custom expressions as Strings, calls to this method should return
    * `UTF8Type.instance`. If the index implementation does not support custom expressions, then it should
    * return null.
    *
  ...
  /** Creates an new indexer object for updates to a given partition.
    ...
    * @param columns         the regular and static columns the created indexer will have to deal with. This can be
    *                        empty as an update might only contain partition, range and row deletions, but the indexer
    *                        is guaranteed to not get any cells for a column that is not part of { @code columns}.
    ...
    * @return the newly created indexer or `null` if the index is not interested by the update (this could be because
    *         the index doesn't care about that particular partition, doesn't care about that type of transaction, ...).
    */
  ...
  /** Return a function which performs post processing on the results of a partition range read command. In future,
    ...
    */
  override def postProcessorFor(command: ReadCommand): BiFunction[PartitionIterator, ReadCommand, PartitionIterator] = {
    (partitions: PartitionIterator, command: ReadCommand) => service.postProcess(partitions, command)
  }
  ...
  /** Factory method for query time search helper. Custom index implementations should perform any validation of query
    ...
    */
  override def searcherFor(command: ReadCommand): org.apache.cassandra.index.Index.Searcher = {
...
object Index {
  ...
  /** Validates the specified index options.
    ...
    */
  def validateOptions(options: java.Map[String, String], metadata: CFMetaData): java.Map[String, String] = {
