...
case class Column[A](
    ...
    deletionTime: Int = NO_DELETION_TIME,
    value: Option[A] = None) {
  ...
  def withDeletionTime(deletionTime: Int): Column[_] = copy(deletionTime = deletionTime)

  /** Returns a copy of this with the specified value. */
  def withValue[B](value: B): Column[B] = copy(value = Option(value))
  ...
  def isDeleted(timeInSec: Int): Boolean = value.isEmpty || deletionTime <= timeInSec
...
object Column {
  ...
  /** The deletion time in seconds for columns that shouldn't be deleted. */
