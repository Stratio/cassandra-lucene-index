...
/** [[CloseableIterator]] for retrieving Lucene documents satisfying a query.
  *
  * @param searchers the Lucene index searcher
  * @param releaseSearchers a function for releasing the searcher
...
class PartitionedDocumentIterator(
    searchers: List[IndexSearcher],
    releaseSearchers: List[() => Unit],
    afterTerms: List[Option[Term]],
    indexSort: Sort,
    querySort: Sort,
    query: Query,
    limit: Int,
    fields: java.util.Set[String]) extends Iterator[(Document, ScoreDoc)] with
  AutoCloseable with
  Logging with
  Tracing {
  ...
  private[this] val offsets = searchers.map(_ => 0).toArray
  ...
  private[this] def fetch() = {
    try {
      ...
      offsets.indices.foreach(i => offsets(i) = offsets(i) + topFieldDocs(i).scoreDocs.length)
  ...
  override def close(): Unit = {
    if (!closed) try releaseSearchers.foreach(_.apply()) finally closed = true
  }

}

/** Companion object for [[DocumentIterator]]. */
object PartitionedDocumentIterator {
