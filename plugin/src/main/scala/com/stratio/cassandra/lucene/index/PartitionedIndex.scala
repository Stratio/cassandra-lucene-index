...
class PartitionedIndex(
  ...
  def upsert(partition: Int, term: Term, document: Document) {
    logger.debug(s"Indexing $document with term $term in $name in partition number $partition")
  ...
  def delete(partition: Int, term: Term) {
    logger.debug(s"Deleting $term from $name in partition number $partition")
  ...
  def delete(partition: Int, query: Query) {
    logger.debug(s"Deleting $query from $name in parttion $partition")
