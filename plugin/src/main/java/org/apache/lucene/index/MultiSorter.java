...
public final class MultiSorter {
    ...
    public interface CrossReaderComparator {
        int compare(int readerIndexA, int docIDA, int readerIndexB, int docIDB);
    }
    ...
    private static CrossReaderComparator getComparator(List<CodecReader> readers, SortField sortField) throws IOException {
        switch(sortField.getType()) {
            ...
            case CUSTOM:
                if (sortField instanceof SerializableSortField) {
                    return ((SerializableSortField)sortField).getCrossReaderComparator(readers);
                }
            default:
