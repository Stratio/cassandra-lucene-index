 ...
 * @lucene.experimental
 */
public class CustomSerializableSortFieldsLuceneSegmentInfoFormat extends SegmentInfoFormat {

  /** Sole constructor. */
  public CustomSerializableSortFieldsLuceneSegmentInfoFormat() {
  }
  
  @Override
  public SegmentInfo read(Directory dir, String segment, byte[] segmentID, IOContext context) throws IOException {
    final String fileName = IndexFileNames.segmentFileName(segment, "", CustomSerializableSortFieldsLuceneSegmentInfoFormat.SI_EXTENSION);
    try (ChecksumIndexInput input = dir.openChecksumInput(fileName, context)) {
      Throwable priorE = null;
      SegmentInfo si = null;
      try {
        int format = CodecUtil.checkIndexHeader(input, CustomSerializableSortFieldsLuceneSegmentInfoFormat.CODEC_NAME,
                                                CustomSerializableSortFieldsLuceneSegmentInfoFormat.VERSION_START,
                                                CustomSerializableSortFieldsLuceneSegmentInfoFormat.VERSION_CURRENT,
                                                segmentID, "");
        ...
        if (numSortFields > 0) {
          ...
          for(int i=0;i<numSortFields;i++) {
            ...
            case 4:
              ...
              break;
            case 5:
              sortType= SortField.Type.CUSTOM;
              break;
            default:
            ...
            if (sortType!=SortField.Type.CUSTOM) {
              ...
              String customSerializableSortField=input.readString();
              Class<?> c = Class.forName(customSerializableSortField);
              SerializableSortField object = (SerializableSortField)c.newInstance();
              object = object.read(input);
              sortFields[i]=object;
            }
          }
  ...
  public void write(Directory dir, SegmentInfo si, IOContext ioContext) throws IOException {
    final String fileName = IndexFileNames.segmentFileName(si.name, "", CustomSerializableSortFieldsLuceneSegmentInfoFormat.SI_EXTENSION);

    try (IndexOutput output = dir.createOutput(fileName, ioContext)) {
      // Only add the file once we've successfully created it, else IFD assert can trip:
      si.addFile(fileName);
      CodecUtil.writeIndexHeader(output,
                                 CustomSerializableSortFieldsLuceneSegmentInfoFormat.CODEC_NAME,
                                 CustomSerializableSortFieldsLuceneSegmentInfoFormat.VERSION_CURRENT,
                                 si.getId(),
                                 "");
      ...
      for (int i = 0; i < numSortFields; ++i) {
        ...
        if (sortField.getType()!=SortField.Type.CUSTOM) {
          // write missing value
        ...
        } else { //custom Serializable_SortField
          output.writeString(sortField.getClass().getName());
          ((SerializableSortField)sortField).write(output);
        }
      }
  ...
  static final String CODEC_NAME = "CustomSerializableSortFieldsLuceneSegmentInfo";
