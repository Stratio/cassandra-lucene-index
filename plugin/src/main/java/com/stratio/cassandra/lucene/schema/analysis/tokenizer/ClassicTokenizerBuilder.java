 ...
 */
public class ClassicTokenizerBuilder extends TokenizerBuilder<ClassicTokenizer> {
    ...
    /**
     * Builds a new {@link ClassicTokenizerBuilder} using the specified maxTokenLength.
     *
     * @param maxTokenLength if a token length is bigger that this, token is split at max token length intervals.
     */
    @JsonCreator
    public ClassicTokenizerBuilder(@JsonProperty("max_token_length") Integer maxTokenLength) {
        this.maxTokenLength = getOrDefault(maxTokenLength, DEFAULT_MAX_TOKEN_LENGTH);
    }

    /** {@inheritDoc} */
    @Override
    public ClassicTokenizer buildTokenizer() {
