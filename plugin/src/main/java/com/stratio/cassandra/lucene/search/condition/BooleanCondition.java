    ...
    public BooleanCondition(Float boost, List<Condition> must, List<Condition> should, List<Condition> not) {
        ...
        if (this.must.isEmpty() && this.should.isEmpty() && (!this.not.isEmpty())) {
            throw new IllegalArgumentException(
                    "Invalid Boolean Query: lucene does not accept pure NOT queries: http://lucene.apache.org/core/2_9_4/queryparsersyntax.html#NOT add a 'must : [{ type : \"match_all\" }]' in your query");
        }
